    ...
    for row in rows:
        ...
        if cols and cols[0]:
            ...
            if ("@" in country) or (" _at_ " in country):
                country = ''
            afil = cols[2]
            attendees.append((fname, lname, afil, country))
...
def parse_lname_fname(soup):
    ...
    for row in rows:
        cols = [col.text for col in row.find_all('td')]
        if cols:
            lname, fname, country, afil = cols[0], cols[1], cols[2].strip(), cols[3]
...
def parse_table_t1(soup):
    ...
    for row in rows:
        ...
        if cols:
            if len(cols) >= 4:
                lname, fname, afil, country = cols[0], cols[1], cols[2], cols[3].strip()
            else:
                lname, fname, afil, country = cols[0], cols[1], cols[2], ''
...
def parse_fname_lname(soup):
    ...
    for row in rows:
        cols = [col.text for col in row.find_all('td')]
        fname, lname, afil, country = cols[0], cols[1], cols[2], country_fixups(cols[3])
        attendees.append((fname, lname, afil, country))
...
def parse_empty_fname_lname(soup):
    ...
    for row in rows:
        cols = [col.text for col in row.find_all('td')]
        fname, lname, afil, country = cols[1], cols[2], cols[3], country_fixups(cols[4])
        attendees.append((fname, lname, afil, country))
